...
class Trader():
    def __init__(self, socketio, ticker, app_key, secret_key, callback_url = "https://127.0.0.1", debug = True):
        ...
        self.socketio: SocketIO = socketio # Socketio, necessary for sending updates to frontend
        ...
        self.system_message = "Nothing for now!" # Message for frontend
        ...
        self.account_cash = TwoDecimal(account_data["account_balance"]) # Current cash in account
        self.current_holdings = TwoDecimal(account_data["current_holdings"]) # Current amount of stock held in account

        # Start streaming data from Schwab
    ...
    def load_state(self, bucket_name, source_blob_name, destination_file_name):
        ...
        # Need to make sure bot knows last buy/sell price
    ...
    def save_state(self, bucket_name, source_file_name, destination_blob_name):
        ...
        data = self.get_trader_data() # has some extra data but good enough for now
        with open("state.json", "r") as file:
            json.dump(data, file)
